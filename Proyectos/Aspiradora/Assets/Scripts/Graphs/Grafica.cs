...
public class Grafica {
  ...
  public bool AStar(Vertice inicio, Vertice final) {
    // Completar
    List<Vertice> abiertos =
        new List<Vertice>(); // Lista de nodos que aun no se han explorado
    List<Vertice> cerrados = new List<Vertice>(); // Lista de nodos ya
                                                  // explorados
    ...
    while (abiertos.Count > 0) { // mientras la lista de abiertos no sea vacia /
                                 // mientras no hayamos explorado todo
      int i = menorF(abiertos);
      Vertice actual;
      actual = abiertos[i]; // nuestro nodo actual es el primero en la lista de
                            // abiertos
      if (actual.id == final.id) {
        reconstruirCamino(inicio, final);
        return true;
      }
      abiertos.RemoveAt(i); // una vez que ya pasamos por nuestro nodo visitado
                            // lo pasamos a cerrados
      cerrados.Add(actual);

      foreach (Vertice vecino in actual.vecinos) {
        if (cerrados.IndexOf(vecino) > -1) {
          continue; // si el un vecino del nodo actual esta la lista de
                    // cerrados, nos saltamos esta iteracion
        }
  ...
  float distancia(Vertice a, Vertice b) {
    ...
    float distancia =
        (float)Math.Sqrt(distanciaX * distanciaX + distanciaY * distanciaY +
