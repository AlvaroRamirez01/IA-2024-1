...
public class ComportamientoAutomatico : MonoBehaviour
{
    ...
    public enum State { MAPEO, DFS, ASTARCARGA, CONTINUARMAPEANDO, CARGANDO }
    ...
    /**
     * PASOS PARA EL DFS
     * 1.- Colocar un vértice (meterlo a la pila 'ColocarNodo' ya lo mete a la
     * pila 2.- Sacar de la pila, e intentar poner mas vértices 3.- Hacer
     * backtrack al siguiente vértice en la pila 4.- Repetir hasta vaciar la pila
     */
    void UpdateMAPEO()
    {
        ...
        if (fp)
        {
            ...
            { // con cada popStack() sacamos el nodo
    ...
    void UpdateAstarCarga()
    {
        ...
        else
        {
            ...
            if (indiceCamino != camino.Count)
            {
                ...
                if (Vector3.Distance(sensor.Ubicacion(),
                                     camino[indiceCamino].posicion) >=
                    0.04f)
                { // si no se ha llegado al vertice
    ...
    void continuarMapeando()
    {
        if (indiceCamino !=
            -1)
        { // si todavia no hemos llegado al vertice en el que nos quedamos
            if (indiceCamino == camino.Count)
            {
                indiceCamino--;
            }
